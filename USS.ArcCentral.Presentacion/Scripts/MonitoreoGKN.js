...
function moni_LoadFormGKN() {
    var html = "<style> .tdCabDet{background-color: #cdcdcd;color: #6f6f6f;} </style>" +
        ...
        "               <button class='btn btn-sm btn-success' onclick='moni_listarMonitoreoGKN();'> <span class='fa fa-search fa_new'></span> Listar</button>" +
        "            </div>" +
        "            <div class='x_content'>" +
        ...
        "                           <select class='form-control' id='cboTipoSolicitud' onchange='moni_LoadForm_lista_impresion_GKN()'>" +
        ...
        "                           <select class='form-control' id='cboCanalOperacion' onchange='moni_LoadForm_lista_impresion_GKN()'>" +
        "                               <option value='-'>Todos</option>" +
        "                               <option value='POS'>POS: Terminal de venta</option>" +
        "                               <option value='WEB'>WEB: Canal Web</option>" +
        /*"                               <option value='IB'>IB: Banca por internet (ViaGKN)</option>" +
        "                               <option value='FI'>FI: Ventanilla</option>" +
        "                               <option value='PZ'>PZ: Agente GKN</option>" +
        ...
        "                    <table class='display' id='tabla_list_GKN'>" +
        "                        <thead>" +
        "                            <tr>" +
        "                               <th>N°</th>" +
        "                               <th>Tipo</th>" +
    ...
    moni_listarMonitoreoGKN();

    $("#frmContenido1").css("display", "none");

}

function moni_listarMonitoreoGKN() {
    ...
    $.ajax({
        type: "POST",
        url: "../Services/srvGeneral.svc/Get_Listado_ConsultasGKN",
        ...
        success: function (response) {
            ...
            arrayMonitoreoGKN = response.d;
            moni_LoadForm_lista_impresion_GKN();
            //console.log(arrayMonitoreoGKN);
            /*var print = "";
            var print1 = "";
            var print2 = "";
...
function moni_LoadForm_lista_impresion_GKN() {
    ...
    if (arrayMonitoreoGKN.length > 0) {
        ...
        for (var i in arrayMonitoreoGKN) {
            validador = 0;
            
            if (cboTipoSol > 0) {
                if (arrayMonitoreoGKN[i]['nSolTipo'] == cboTipoSol ) {
                    validador++;
                }
            }else{
                validador++;
            ...
            if (cboCanalOpe != '-') {
                if (arrayMonitoreoGKN[i]['canalOperacion'] == cboCanalOpe ) {
                    validador++;
                }
            }else{
                validador++;
...
/*
            if (cboProceso > 0) {
                if (arrayMonitoreoGKN[i]['nProceso'] == cboProceso ) {
                    validador++;
                }
            }else{
                validador++;
            ...
            if (cboEstadoDoc > 0) {
                if (arrayMonitoreoGKN[i]['nSGCDocEstado'] == cboEstadoDoc ) {
                    validador++;
                }
            }else{
                validador++;
            ...
            if (validador == 2) {
                var stilo = "";
                if (arrayMonitoreoGKN[i]['nSolTipo'] == 2) {
                    cantidadRecibos++;
                    if (arrayMonitoreoGKN[i]['nProcesado'] == 1) {
                        stilo = "style='background-color: #65b068;color: white;'";
                        cantidadRecibosPagados++;
                        totaldepago += parseFloat(arrayMonitoreoGKN[i]['importeDeudaPagada']);
                    }else{
                ...
                html1 += "<tr>";
                html1 += "    <td>" + arrayMonitoreoGKN[i]['nSolCodigo'] + "</td>";
                html1 += "    <td "+stilo+">" + arrayMonitoreoGKN[i]['cSolTipo'] + "</td>";
                html1 += "    <td>" + arrayMonitoreoGKN[i]['canalOperacion'] + "</td>";
                html1 += "    <td>" + arrayMonitoreoGKN[i]['fechaOperacion'] + "</td>";
                html1 += "    <td>" + arrayMonitoreoGKN[i]['horaOperacion'] + "</td>";
                html1 += "    <td>" + arrayMonitoreoGKN[i]['numeroReferenciaDeuda'] + "</td>";
                html1 += "    <td>" + arrayMonitoreoGKN[i]['numeroDocumento'] + "</td>";
                html1 += "    <td>" + arrayMonitoreoGKN[i]['numeroOperacionRecaudos'] + "</td>";
                html1 += "    <td>" + arrayMonitoreoGKN[i]['importeDeudaPagada'] + "</td>";
                //html1 += "    <td>" + arrayMonitoreoGKN[i]['numeroOperacionRecaudos'] + "</td>";
                html1 += "    <td>" + arrayMonitoreoGKN[i]['formaPago'] + "</td>";
                html1 += "    <td>" + arrayMonitoreoGKN[i]['codigoMoneda'] + "</td>";
                //html1 += "    <td>" + arrayMonitoreoGKN[i]['nProcesado'] + "</td>";
                //html1 += "    <td><a href='https://campus.uss.edu.pe/Campus/DocInternos/DocCalidad/"+arrayMonitoreoGKN[i]['cDocumento']+"' target='_blank' class=''><span class='fa fa-file-pdf-o'></span> Ver</a></td>";
                //html1 += "    <td>"+arrayMonitoreoGKN[i]['nVersion']+"</td>";
                /*if (arrayMonitoreoGKN[i]['nSolTipo'] == 2 ) {
                    html1 += "       <td><button onclick='moni_verPago(" + arrayMonitoreoGKN[i]['nSolCodigo'] + ")' class='btn_acc'><i class='fa fa-eye' title='Ver'></i></button></td>";
                }else{
                    html1 += "       <td></td>";
                }*/
                //html1 += "       <button onclick='moni_calificarProveedor("+arrayMonitoreoGKN[i]['nProvCodigo']+")' class='btn_acc'><i class='fa fa-' title='Ver'></i></button></td>";
                html1 += "</tr>";
            }
        }

        var table = $('#tabla_list_GKN').DataTable();
        ...
        $('#tabla_list_GKN').DataTable({ "order": [[ 0, "desc" ]] });
    }else{
        var table = $('#tabla_list_GKN').DataTable();
        table.destroy();
        ...
        onOpen: function () {
            if (nSolCodigo > 0) {
                for (var i in arrayMonitoreoGKN) {
                    ...
                    if (arrayMonitoreoGKN[i]['nSolCodigo'] == nSolCodigo) {
