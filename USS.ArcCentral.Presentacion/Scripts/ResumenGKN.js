...
function resuGKN_LoadForm() {
    var html = "<style> .tdCabDet{background-color: #cdcdcd;color: #6f6f6f;} </style>" +
        ...
        "        <div class='x_panel'>" +
        "            <div class='x_title'>" +
        "               <button class='btn btn-sm btn_pri_nuevo' onclick='resuGKN_listarMonitoreo();'> <span class='fa fa-refresh fa_new'></span> Actualizar</button>" +
        "            </div>" +
        "            <div class='x_content'>" +
        "                <div>" +
        ...
        "                                    <td colspan='2'>POS: Terminal</td>" +
        "                                    <td colspan='2'>WEB: Internet</td>" +
        "                                </tr> " +
        "                                <tr> " +
        "                                    <td>S/</td>" +
        "                                    <td>#</td>" +
    ...
    resuGKN_listarMonitoreo();

    $("#frmContenido1").css("display", "none");

}

function resuGKN_listarMonitoreo() {

    var Data = {
    };
    $.ajax({
        type: "POST",
        url: "../Services/srvGeneral.svc/Get_Listado_ResumenGKN",
        ...
        success: function (response) {
            ...
            arrayResumenGKN = response.d;
            resuGKN_LoadForm_lista_impresion();
...
function resuGKN_LoadForm_lista_impresion() {

    // var cboTipoSol = $("#cboTipoSolicitud").val(); 
    // var cboCanalOpe = $("#cboCanalOperacion").val(); 
    //console.log(arrayResumenGKN.length );
    var html1 = '';
    if (arrayResumenGKN.length > 0) {

        for (var i in arrayResumenGKN) {

            if (arrayResumenGKN[i]['Tipo'] == 1) {
                html1 += "<tr>";
                html1 += "    <td>" + arrayResumenGKN[i]['fechaOperacion'] + "</td>";
                html1 += "    <td>" + arrayResumenGKN[i]['tDia'] + "</td>";
                html1 += "    <td>" + arrayResumenGKN[i]['oDia'] + "</td>";
                html1 += "    <td>" + arrayResumenGKN[i]['POS'] + "</td>";
                html1 += "    <td>" + arrayResumenGKN[i]['oPOS'] + "</td>";
                html1 += "    <td>" + arrayResumenGKN[i]['WEB'] + "</td>";
                html1 += "    <td>" + arrayResumenGKN[i]['oWEB'] + "</td>";
                html1 += "</tr>";
            }else{
